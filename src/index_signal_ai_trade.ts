...
const workerId = "with_signals_ai";
...
async function start() {
  ...
  try {
    ...
    if (args.length !== 4) {
      ...
      mainLogger.error(
        "ตัวอย่าง: bun start:signal-ai-trade <MAX_TOTAL_TRADE> <MAX_CONCURRENT_TRADE>  <MINUTE (1|5)> <BUY_AMOUNT>"
      );
      mainLogger.error("ตัวอย่าง: bun start:signal-ai-trade 5 1 10");
      ...
      snapshot.docChanges().forEach((change: any) => {
        if (change.type === "added") {
          ...
          if (currentCount < maxConcurrentTrade) {
            ...
            Bun.spawn(["bun", "src/workers/SignalAiTradeWorker.ts"], {
              env: {
                ...process.env,
...
function logSignalExecution(
  ...
  mainLogger.info(
    `\n🚀 ==================== เริ่มการทำงานกับสัญญาณการซื้อขายด้วย AI ====================
...
function logTradeCompletion(
  ...
  mainLogger.info(
    `\n✅ ==================== การทำงานของสัญญาณการซื้อขายด้วย AI จบลง ====================
